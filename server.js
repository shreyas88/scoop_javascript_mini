let write_yaml = require('write-yaml');
let read_yaml = require('read-data');
...
function CreateComment(url, request) {
    ...
    if (request && request.body) {
      req_comment = request.body.comment;
    }

    if (!req_comment || !req_comment.body || !req_comment.username ||
        !(req_comment.username in database.users) ||
        (req_comment.articleId <= 0) ||
        (req_comment.articleId > database.nextCommentId)) {
          response.status = 400;
          return response;
    }
    ...
    const comment = {
      ...
      body: req_comment.body,
      username : req_comment.username,
      articleId: req_comment.articleId,
      upvotedBy: [],
      downvotedBy:[]
    };
...
function updateComment(url, request) {
    ...
    if (id > database.nextCommentId || id < 0) {
      response.status = 404;
      return response;
    }
...
function deleteComment(url, request) {
  // remove reference from user node
  //
  const id = Number(url.split('/').filter(segment => segment)[1]);
  let response = {};

  if (!id || id > database.nextCommentId || id < 0) {
      response.status=404;
      return response;
  }

  // remove commentid from user which refers to it
  //
  let comment = database.comments[id];
  let user = database.users[comment.username];
  let article = database.articles[comment.articleId];

  // remove commentid from article which refers to it
  //
...
function upvoteComment(url, request) {
  ...
  if (!id || id > database.nextCommentId || id < 0 || !request ||
      !request.body || (!(user in database.users))) {
      response.status=400;
      return response;
  }
  const comment = database.comments[id];
  if (comment.upvotedBy.indexOf(user) == -1) {
    comment.upvotedBy.push(user);
  }

  if (comment.downvotedBy.indexOf(user) != -1) {
    comment.downvotedBy.splice(comment.downvotedBy.indexOf(user),1);
  }
  response.status = 200;
  response.body = {}
  response.body.comment = comment;
...
function downvoteComment(url, request) {
  ...
  const comment = database.comments[id];
  if (comment.downvotedBy.indexOf(user) == -1) {
    comment.downvotedBy.push(user);
  }

  if (comment.upvotedBy.indexOf(user) != -1) {
    comment.upvotedBy.splice(comment.upvotedBy.indexOf(user),1);
  }
  response.status = 200;
  response.body = {}
  response.body.comment = comment;
...
function saveDatabase() {
  write_yaml.sync('.database.yml', database);
};

function loadDatabase() {
  return read_yaml.data.sync('.database.yml');
};
